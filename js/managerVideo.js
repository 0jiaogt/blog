...
function makeTable(obj, res){
	...
	for(var i = 0; i<res.length; i++){
		res_table += `<tr>
			   			<td class="v_id">${res[i].videoId}</td>
			   			<td>${res[i].categoryId}</td>
			   			<td>${res[i].title}</td>
			   			<td>${res[i].src}</td>
			   			<td>${res[i].author}</td>
			   			<td>${res[i].readAccount}</td>
			   			<td>${res[i].replyAccount}</td>
			   			<td>${res[i].like}</td>
			   			<td>${res[i].unlike}</td>
			   			<td>${res[i].time}</td>
						<td><span class="btn btn-danger btn-xs btn-del">删除</span></td>
			   		</tr>`
	}
...
$('.container .panel table').on('click', function(e){
	if(e.target.nodeName.toUpperCase() === 'SPAN'){
		if(e.target.classList.contains('btn-del')){
			...
			reqPa.videoId = pa.getElementsByClassName('v_id')[0].innerText
