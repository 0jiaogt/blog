...
function makeTable(obj, res){
	...
	for(var i = 0; i<res.length; i++){
		res_table += `<tr>
			   			<td class="c_id">${res[i].contributeId}</td>
			   			<td>${res[i].category}</td>
			   			<td>${res[i].title}</td>
			   			<td>${res[i].content}</td>
			   			<td>${res[i].userId}</td>
			   			<td>${res[i].time}</td>
			   			<td><span class="btn btn-danger btn-sm btn-save">通过</span></td>
						<td><span class="btn btn-danger btn-sm btn-del">不通过</span></td>
			   		</tr>`
	}
...
$('.container .panel table').on('click', function(e){
	if(e.target.nodeName.toUpperCase() === 'SPAN'){
		if(e.target.classList.contains('btn-save')){      //通过，先保存再删除
			var pa = e.target.parentNode.parentNode
			reqPa.contributeId = pa.getElementsByClassName('c_id')[0].innerText
			ajax('get', '/resource/deleteResource',reqPa, del_callback)
		}else if(e.target.classList.contains('btn-del')){       //不通过，直接删除
			var pa = e.target.parentNode.parentNode
			reqPa.contributeId = pa.getElementsByClassName('c_id')[0].innerText
			ajax('get', '/resource/deleteResource',reqPa, del_callback)
		}
